...
const SkillsSection: React.FC = () => (
  <section className="py-20">
    <motion.div
      ...
      variants={staggerContainer}
    >
      <motion.h2 variants={fadeInUp} className="text-4xl font-bold font-display mb-12 text-center">Technical <span className="text-blue-500">Skillset</span></motion.h2>
      <div className="grid md:grid-cols-2 gap-8">
        {SKILLS.map((group, idx) => (
          <motion.div 
            ...
            className="p-8 bg-slate-900 border border-slate-800 rounded-3xl hover:border-blue-500/30 transition-colors shadow-lg hover:shadow-blue-900/10"
          >
            ...
            <div className="flex flex-wrap gap-2">
              {group.skills.map((skill, sIdx) => (
                <motion.span 
                  ...
                  whileHover={{ scale: 1.05, backgroundColor: "rgba(59, 130, 246, 0.2)", borderColor: "rgba(59, 130, 246, 0.4)", color: "#60a5fa" }}
                  className="px-3 py-1.5 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-300 transition-all cursor-default"
                >
                  {skill}
                </motion.span>
              ))}
